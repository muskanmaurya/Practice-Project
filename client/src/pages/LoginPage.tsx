...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try{
      ...
      if(response.data.success){
        ...
        // Safe execution loop calling the handler
        handleAuthSuccess(response.data);

      }
    }catch(error){
      console.error("Error occurred while logging in user:", error);
      toast.error("Error occurred while logging in user. Please try again.");
    }finally{
      setLoading(false);
    }
  };
  ...
  const handleAuthSuccess = (data: any) => {
    ...
    // Navigation comes at the very end of the pipeline execution
    navigate("/");
  }
          ...
          <div className="w-full">
            ...
            <form onSubmit={handleSubmit} className="mt-8 space-y-5">
              ...
              <div className="flex justify-start">
                <a href="#forgot" className="text-xs font-medium text-gray-400 hover:text-orange-500 transition-colors">
                  Forgot Password?
                </a>
              </div>
